...
import opennlp.tools.namefind.{NameSampleDataStream,NameFinderME}
...
object NETrainer {
  ...
  def apply(
     ...
     * that serves an input file for building an NER model
     */
    infile:String,
    /*
     * Binary file that specifies the NER model for a certain
     * category
     */
    outfile:String,
    /*
     * The NE category, which is actually person, location & organization
     */
    category:String,
    /*
     * Number of iterations to build a certain NE model
     */
    iterations:Int = 100,
    /*
     * The cutoff used to specify the NE model
     */
    ...
    val trainParameters = ModelUtil.createTrainingParameters(iterations,cutoff)
    
    /*
     * We use the default feature generator here
     */
    val generator:AdaptiveFeatureGenerator = null
    val modelOut = new java.io.BufferedOutputStream(new java.io.FileOutputStream(outfile))
  
    try {
   
      val model = NameFinderME.train("de", category, sampleStream, trainParameters, generator, Map.empty[String,Object])
